...
func TestGFSWeatherEngine(t *testing.T) {
	...
	grid := engine.GetGrid(30.0, 45.0, -75.0, -60.0, 1.0, 1.0, now)
	if len(grid) != 16 || len(grid[0]) != 16 {
		t.Fatalf("Expected 16x16 grid, got %dx%d", len(grid), len(grid[0]))
	}
}
...
func TestLiveNOAAGFSEngine(t *testing.T) {
	...
	// Sample with live provider (with automatic fallback)
	w := liveEngine.GetWind(21.3, -157.8, now)
	if w.TWS < 1.0 || w.TWS > 70.0 {
		t.Fatalf("Expected valid wind speed, got %.2f kts", w.TWS)
	}

	grid := liveEngine.GetGrid(20.0, 25.0, -160.0, -155.0, 2.5, 2.5, now)
	if len(grid) == 0 || len(grid[0]) == 0 {
		t.Fatalf("Expected non-empty grid from LiveNOAAGFSEngine")
	}
}
